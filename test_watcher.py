...
# ===== v0.1.11 model_task 单选下拉（固定推荐任务集，不动态枚举 Host）=====


def test_get_webui_config_schema_model_task_is_select(tmp_path):
    """schema 注入：model_task 恒为单选 select（无自由文本输入），
    默认 choices 恰为 replyer / planner / utils；其余 schema 元数据不动。"""
    plugin = _make_plugin(tmp_path)
    schema = plugin.get_webui_config_schema()
    field = schema["sections"]["llm"]["fields"]["model_task"]
    assert field["type"] == "select"
    assert field["ui_type"] == "select"
    assert field["choices"] == ["replyer", "planner", "utils"]
    assert field["label"] == "模型任务"
    assert field["default"] == "replyer"
    assert "multiple" not in field  # 单选


def test_model_task_legacy_custom_value_displayed_not_rewritten(tmp_path):
    """旧自定义任务值不在推荐集：仅追加到显示列表末尾用于选中显示；
    配置值不被改写、不 fallback 到 replyer。"""
    plugin = _make_plugin(tmp_path)
    plugin.config.llm.model_task = "my-custom-task"
    assert plugin.config.llm.model_task == "my-custom-task"
    field = plugin.get_webui_config_schema()["sections"]["llm"]["fields"]["model_task"]
    assert field["ui_type"] == "select"
    assert field["choices"] == ["replyer", "planner", "utils", "my-custom-task"]
    assert plugin.config.llm.model_task == "my-custom-task"  # 不改写
...
def test_llm_generate_still_uses_configured_model_task(tmp_path):
    """固定候选不影响 generate：实际调用仍显式使用配置的 model_task
    （含不在推荐集中的旧自定义值）。"""
    plugin = _make_plugin(tmp_path)
    plugin.config.llm.model_task = "my-custom-task"
    _wire_providers(plugin, fx=_FX_GOLDEN)
    ...
    assert "中文翻译：" in plugin._ctx.send.sent_messages[0][1]


def test_model_task_schema_static_classmethod_unchanged():
    """类级静态 schema（host rebuild / 默认导出路径）不受实例渲染影响：
    字段名、类型、默认值与 0.1.10 完全一致（str、无 Literal 强校验）。"""
    from maibot_sdk.config import generate_plugin_config_schema
