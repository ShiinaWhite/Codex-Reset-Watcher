...
0.1.12 新增「上游推送镜像」：插件同时读取数据源面向用户的推送通知内容（与其浏览器推送同源）。只要上游决定向用户推送一条 Banked 告警，插件就把同一条告警镜像到 QQ——标题、原文翻译、原帖结构与其它提醒完全一致。触发只看上游已经做出的推送决策，插件不做第二层语义判断，也不会因为状态是 `unknown` 就静默。

**已知限制（诚实说明）**：这个推送面是"最新一条"性质，上游不提供历史记录。如果插件停机或两次检查之间，上游恰好推送了某条 Banked 消息、随后又被更新的消息覆盖，这条消息将无法找回。0.1.12 只保证「运行期间仍然可见的推送不漏」，不承诺离线恢复；离线期间的补偿需要数据源提供带历史/游标的告警接口，属于上游能力范围。
